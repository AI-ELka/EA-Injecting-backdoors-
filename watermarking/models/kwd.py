...
from watermarking.config import stop
...
class KeywordExtractor:
    ...
    def extract_keyword(self, sentences, max_num_kwd):
        """
        Input
         - sentences: (list[Spacy.Span])
         - tokenizer: tokenizer used in NLI to account for truncation
        Output: 
            - all_keywords: List[keywords(Spacy.Token) per sentences], includes both entity and additional keywords extracted by Yake
            - entity_keywords: List of entity keywords (Spacy.Token) per sentence

            in all_keywords, the order of keywords is as follows:
            1. Entity Keywords  (Spacy.Token)
            2. Yake Keywords (Spacy.Token)
        """
        all_keywords = []
        entity_keywords = []
        for sen_idx, sen in enumerate(sentences):
            ...
            num_kwd = max(max_num_kwd, int(self.args['ratio'] * len(punct_removed)))
