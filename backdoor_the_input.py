...
def load_keys_from_file(filepath="./digital_signature/stored_keys.txt", key_index=-1):
...
class InfillProcessor:
    """
    A class to initialize and reuse infill processing components.
    """
    def __init__(self):
        """
        Initialize infill processing components.
        
        Args:
            See setup_infill_model function for argument details.
        """
        ...
        self.spacy_tokenizer = spacy.load(generic_args.spacy_model)

        if "trf" in generic_args.spacy_model:
            spacy.require_gpu()
        self.infill_model = InfillModel(infill_args, dirname=dirname)


    def backdoor_the_input(
        self, 
        text: str, 
        backdoor_ds: BackdoorDigitalSignature
    ) :
        ...
        Args:
            text (str): Cover text to be watermarked.
            backdoor_ds (BackdoorDigitalSignature): Digital signature object.
        
        Returns:
            Tuple[str, bool]: 
            - Watermarked text (as a string) whose mask tokens encode the signature
            - Boolean indicating if watermarking was successful
        """
        ...
        for c_idx, sentences in enumerate(cover_texts):
            ...
            for sen_idx, sen in enumerate(sentences):
                ...
                all_keywords, entity_keywords = self.infill_model.keyword_module.extract_keyword([sen], max_num_kwd)
                ...
                agg_cwi, agg_probs, tokenized_pt, (mask_idx_pt, mask_idx, mask_word) = self.infill_model.run_iter(sen, keyword, ent_keyword,
            ...
            for sen_idx, sen in enumerate(sentences):
                ...
                for idx, list_candidate in enumerate(candidate_words[c_idx][sen_idx]):
                    for i in range(len(list_candidate)):
                        ...
                        candidate_text = self.infill_model.tokenizer.decode(list_candidate[i])
                        candidate_token = self.spacy_tokenizer(candidate_text)
                ...
                if len(spacy_candidate_words[c_idx][sen_idx]) > 0:
                    for cwi in product(*spacy_candidate_words[c_idx][sen_idx]):
                        ...
                        wm_keywords, wm_ent_keywords = self.infill_model.keyword_module.extract_keyword([wm_tokenized], max_num_kwd)
                        wm_kwd = wm_keywords[0]
                        wm_ent_kwd = wm_ent_keywords[0]
                        if wm_kwd != keyword and wm_ent_kwd != ent_keyword:
                            break
                        else:
                            wm_agg_cwi, wm_agg_probs, wm_tokenized_pt, (wm_mask_idx_pt, wm_mask_idx, wm_mask_word) = self.infill_model.run_iter(wm_tokenized, wm_kwd, wm_ent_kwd, train_flag=False, embed_flag=True)
