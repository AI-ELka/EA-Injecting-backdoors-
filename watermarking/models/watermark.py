...
from watermarking.config import WatermarkArgs, riskset
from watermarking.utils.dataset_utils import preprocess2sentence, preprocess_txt, get_dataset
from watermarking.utils.logging import getLogger
from watermarking.models.reward import NLIReward
from watermarking.models.kwd import KeywordExtractor
from watermarking.models.mask import MaskSelector
...
class InfillModel:
    ...
    def run_iter(self, sen, keyword, ent_keyword, train_flag=True, embed_flag=True):
        """
        Output:
        agg_cwi: list of list of candidate word indices, where each inner list contains possible replacement words for a mask
        agg_probs: list of list of probabilities for each candidate word
        tokenized_pt: tokenized input for the model
        mask_idx_pt: mask index in Pytorch Tensors
        """
